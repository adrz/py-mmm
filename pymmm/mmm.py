...
class mmm():
    ...
    def predict(self, observations):
        ...
        trans_matrix = np.reshape(trans_matrix,(self.dim_state_space*self.dim_state_space, n_obs))

        log_A       = np.reshape(log_A, (self.dim_state_space*self.dim_state_space, self.n_cluster))
    ...
    def fit(self, observations=[], max_iter=1000, threshold = 1e-8, verbose=False):
        ...
        for iter in range(max_iter):
            ...
            if np.isnan(self.transition_probs).any():
                print('A na')
                break
            self.prior   = normalize(np.sum(self.posterior, 1).reshape((self.n_cluster,1)))
    ...
    def plot_transition_matrix(self, cluster=0, title="",\
        ...
        cax = ax.matshow(data, interpolation='nearest',vmin=0, vmax=1, cmap='Reds')
...
def get_list_actions_v2(df_list_sessions, possible_actions=['homepage','mediation','sru','download','navigation']):
    ...
    for session in df_list_sessions:
        ...
        action = np.where(np.array(session).reshape(dim_resize, 1))[0]%nactions
